...
static void pwmOut9pcb(PWMDriver *pwmp)
{
    (void)pwmp;
    if ((pwmp->enabled & (1 << 0)) && (pwmp->tim->CCR[0] > 0))
        palSetLine(LINE_PF9_IN);
}
static void pwmOut10pcb(PWMDriver *pwmp)
{
    (void)pwmp;
    if ((pwmp->enabled & (1 << 0)) && (pwmp->tim->CCR[0] > 0))
        palSetLine(LINE_PF10_IN);
}
...
static void pwmOut9cb(PWMDriver *pwmp)
{
    (void)pwmp;
    if (pwmp->enabled & (1 << 0))
        palClearLine(LINE_PF9_IN);
}
static void pwmOut10cb(PWMDriver *pwmp)
{
    (void)pwmp;
    if (pwmp->enabled & (1 << 0))
        palClearLine(LINE_PF10_IN);
}
//===============================================
...
static const PWMConfig pwm14Cfg = {
    .frequency = 1000000,
    .period = 10000,
    .callback = pwmOut9pcb,
    .channels = {
